...
>(({ side = 'right', className, children, hideCloseButton = false, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      {/* Hidden fallback title for accessibility — overridden when SheetTitle is used */}
      <SheetPrimitive.Title className="sr-only">Dialog</SheetPrimitive.Title>
      {!hideCloseButton && (
        <SheetPrimitive.Close className="absolute end-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-neutral-950 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-neutral-100">
          <X className="h-4 w-4" />
