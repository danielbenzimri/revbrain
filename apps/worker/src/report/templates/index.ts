...
function renderCover(data: ReportData): string {
  const rows: string[][] = [
    ...
    ['Adv. Approvals Version', data.metadata.sbaaVersion ? escapeHtml(data.metadata.sbaaVersion) : 'Not installed'],
...
function renderConfigDomain(data: ReportData): string {
  ...
  <div class="page-break">
    ...
    ${
      dormant.length > 0
        ? `<p><em>${dormant.length} of ${data.configurationDomain.productCatalog.length} product families show zero quoting activity in the 90-day window — including ${dormant.slice(0, 3).map((d) => d.name).join(', ')}${dormant.length > 3 ? ', and others' : ''}. This may indicate dormant catalog segments, seasonal patterns, or a narrow active use-case.</em></p>`
        : ''
    }
...
function renderDataQuality(data: ReportData): string {
  ...
  <div class="page-break">
    ...
    ${
      data.dataQuality.fieldCompleteness.length > 0 && data.dataQuality.fieldCompleteness.some((f) => f.totalFields > 0)
        ? table(
            [
              'Object',
              'Total Fields',
