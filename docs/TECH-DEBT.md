...
| --------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **Edge Function cold start (~3s)**      | Supabase Edge Runtime module loading                         | PostgREST repos + dynamic imports implemented but Deno still loads `drizzle-orm` transitively via import map. Warm requests improved (3.6s→2s). Full fix requires either: (1) separate edge function entry point that doesn't import drizzle-orm, or (2) Supabase lazy module loading support. | Medium — impacts first request after idle period                  |
